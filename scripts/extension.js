Hooks.once('init', async () => {
    Handlebars.registerHelper('splitAttributes', function(attributesString) {
        ...
        const parts = attributesString.split(/<br\s*\/?>/i)
            .map(p => p.trim())
            .filter(p => p);
        ...
        const buildList = (items, className) => {
            ...
            return items.map(item => `<div class="${className}">${item}</div>`).join('');
        };
    ...
    const overridePath = "modules/dovers-greybearded-qol-enhancements/templates/select-class-override.html";
    const originalKey = "modules/mosh-greybearded-qol/templates/character-creator/select-class.html";
    ...
    try {
        const module = await import('/modules/mosh-greybearded-qol/scripts/character-creator/select-class.js');
        ...
        if (ClassSelectorApp) {
             ...
             ClassSelectorApp.prototype._updateSelectionUi = function() {
                ...
                root.querySelectorAll(".class-list-item, .class-card").forEach(item => {
                ...
                const confirmBtn = root.querySelector('[data-action="confirm"]') || root.querySelector('#confirm-button');
                if (confirmBtn) {
